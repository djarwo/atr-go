...
func (s UserRepository) Create(obj models.User, userOutlet string, usercustomerid string) (*models.User, int, error) {
...
func (s UserRepository) Update(id string, obj models.User, userOutlet string, usercustomerid string) (*models.User, int, error) {
	err := s.db.DB.Set("gorm:auto_preload", true).Where(library.GetBusiness()).First(&s.obj, id).Error
	if err != nil {
		s.r.StatusCode = http.StatusInternalServerError
		return nil, s.r.StatusCode, err
	}
...
func (s UserRepository) Delete(id string) (*models.User, int, error) {
	err := s.db.DB.Where("id = ?", id).Where(library.GetBusiness()).Delete(&s.obj).Error
